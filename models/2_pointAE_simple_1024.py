...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    #                      scope='dp1')
    net3 = tf_util.fully_connected(net3, 4096, bn=True, is_training=is_training,
                                  scope='fc2', bn_decay=bn_decay)
    print(net3.get_shape().as_list())
    #net = tf_util.dropout(net, keep_prob=0.7, is_training=is_training,
    #                      scope='dp2')
    net3 = tf_util.fully_connected(net3, 3072, activation_fn=None, scope='fc3')
    print(net3.get_shape().as_list())
    net3 = tf.reshape(net3, [batch_size, 1024,3])
    print(net3.get_shape().as_list())
...
def get_loss(pred, label, goal_pcs, reg_weight=0.001):
    ...
    for i in range(1024):
        ...
        for j in range(k):        
            point2 = rec[1,:]
            #print(tf.reduce_sum(tf.square(point1 - point2)))
    """
